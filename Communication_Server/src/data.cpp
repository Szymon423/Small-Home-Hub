...
namespace Data {
            ...
            for (const auto& item : requestBody["Data"]) {
                ...
                switch (shmem->definitions[position].dataType) {
                    ...
                    case Signals::DataType::Text: {
                        ...
                        Logger::trace("Updating signal with ID: {} with value: {}", ID, value);
                        Utilities::Text::CopyStringToArray(value, shmem->currentValues[position].text);
                        break;
                    }
                    default: {
    ...
    void get(Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response) noexcept {
        try {
            ...
            // TODO: here needs to be added data collecting rutine to for allowing sending data to nodes
